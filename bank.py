    ...
    if user_data:
        if request.method == 'POST':
            ...
            if not cursor.fetchone():
                # Insert a new row
                cursor.execute("INSERT INTO accounts (user_id, balance, account_type) VALUES (%s, %s, %s)", (user_data['user_id'], 0, account_type))
...
def check_balance():
    ...
    if user_data:
        ...
        balance = result[0] if result is not None else 0
